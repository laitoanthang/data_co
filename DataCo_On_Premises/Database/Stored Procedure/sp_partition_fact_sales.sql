...
SELECT @start_date = MIN(CONVERT(INT, CONVERT(VARCHAR,CONVERT(DATETIME, order_date_dateorders,101),112))) FROM DB_staging.dbo.dataco_orders;
SELECT @end_date = MAX(CONVERT(INT, CONVERT(VARCHAR,CONVERT(DATETIME, order_date_dateorders,101),112))) FROM DB_staging.dbo.dataco_orders;
